...
where $X$ is a binomial random variable obtained from 10 trials and success probability of 0.5. This means $n = 10$, $p = 0.5$, and $k = 3$.
\begin{align*}
...
\textbf{Hint:} Remember that
\[P(X = k) = \binom{n}{k}p^k(1-p)^{n-k}\]
for any nonnegative integer $k \le n$. Since we have implemented $\binom{n}{k}$ with \colorbox{CornflowerBlue!20}{\texttt{choose}}, we can 
calculate $P(X \le k)$ right away!
\begin{flushright}
    [4 marks]
\end{flushright}
\item[\textbf{F.}]
